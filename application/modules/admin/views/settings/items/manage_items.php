                                        ...
                                        <div class="col-lg-6 text-center" style="padding:0rem">
                                            <div class="table-responsive" style="line-height:0.5">
                                                <table id="datatable" class="table v-middle p-0 m-0 box" data-plugin="dataTable">
                                                    ...
                                                    <tbody>
                                                        <?php
                                                        $data_req = json_decode($item_details);
                                                        if ($data_req->code == SUCCESS_CODE):
                                                            $sno = 1;
                                                            foreach ($data_req->item_result as $data_res):
                                                                $id=$data_res->item_unique_id;
                                                                ?>
                                                                <tr>
                                                                    <td><?php echo $sno; ?>
                                                                    <input type="checkbox">
                                                                    </td>
                                                                    <td><?php echo fetch_ucfirst($data_res->item_name); ?></td>
                                                                    <td><?php echo strtoupper($data_res->item_code); ?></td>
                                        ...
                                        <div class="col-lg-5 pull-right" style="padding:0rem">
                                            <form action="" method="post" name="bulk_insert_form" id="bulk_insert_form" enctype="multipart/form-data" >
                                                ...
                                                <div class="form-row col-lg-12">
                                                    ...
                                                    <tr>
                                                    <td>Choose Food Type</td>
                                                    <td colspan="3">
                                                            <select name="food_type" id="food_type" class="col-lg-6 form-control">
                                                            <option value="0">Choose Food Type</option>
                                                            <?php
                                                            $foodtype_req=json_decode($food_type_details);
                                                            if($foodtype_req->code==SUCCESS_CODE){
                                                                foreach($foodtype_req->foodtype_result as $food_res){ ?>
                                                                    <option value="<?php echo $food_res->unique_id; ?>"><?php echo fetch_ucfirst($food_res->title); ?></option>
                                                                <?php  }
                                                            }
                                                            ?>
                                                            </select>
...
<script type="text/javascript">
    $('#bulk_insert_form').on('submit', function (e) {
      ...
      */
        ...
        if(food_type==''){
            $('#food_type_error').html('Please choose food type').css({'color':'red'}); str=false;
        }
        if (str == true)
        {
            ...
            $.ajax({
                ...
                url: "<?php echo ADMIN_LINK; ?>Settings/insertItems",
                contentType: false,
                cache: false,
                processData: false,
                success: function (s) {
